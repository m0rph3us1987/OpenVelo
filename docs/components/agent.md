...
- `implement()` — Mode-switches the same plan+implement session to `code` mode and sends `implementer.txt` (a short reminder with a `{{PLAN_PATH}}` pointer). The blueprint plan, SKILLS, architecture, and story are already in conversation history from the planning turn; the implementer can re-read the plan from disk if context compacts.
