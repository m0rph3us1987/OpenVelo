...
|-------|-----------|--------|---------|-------|
| `init` | `''`, `'mounting'`, `'starting'`, `'error'` | (none — uses `mountAndCheckout` and `serveRegistry`) | `stage-init.ts` | Cleans `chatDir`, mounts the project repo via GBFS, writes both `opencode.json` and `kilo.json` (whitelisting `/SKILLS` and `/tmp`), spawns `kilo serve`, transitions to `analyzing`. Owns its own `addPatToUrl()`. |
