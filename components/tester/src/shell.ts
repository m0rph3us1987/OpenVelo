...
/**
 * Remove watch-mode flags from a build/test command so it runs once and
 * exits instead of blocking forever. Strips `--watch`, `--watchAll`, and a
 * standalone `-w` flag. Collapses any resulting double spaces. The returned
 * command is functionally equivalent for a single CI-style run.
 */
export function stripWatchFlags(command: string): string {
    let stripped = command
        .replace(/\s--watchAll(=\S+)?\b/g, ' ')
        .replace(/\s--watch(=\S+)?\b/g, ' ')
        .replace(/\s-w\b/g, ' ');
    // Collapse runs of whitespace introduced by removals and trim.
